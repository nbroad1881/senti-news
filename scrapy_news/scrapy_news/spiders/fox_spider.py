...
class MySpider(scrapy.Spider):
    ...
    base = 'https://api.foxnews.com/v1/content/search?q=' + search_q + '&fields=date,description,title,url,image,' \
                                                                       'type,' \
                                                                       'taxonomy&section.path=fnc&type=article' \
                                                                       '&min_date=' + \
    ...
    def parse(self, response):
        ...
        with open('fox_urls2.txt', 'a') as f:
            for d in docs:
                text = d['title'] + '\n' + d['date'] + '\n' + d['url'][0] + '\n'
                print(text)
                f.write(text)
