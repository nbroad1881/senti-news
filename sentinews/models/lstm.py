...
def evaluate(model_dir, texts, max_length=100):
    nlp = spacy.load("en_vectors_web_lg")
    nlp.add_pipe(SentimentAnalyser.load(model_dir, nlp, max_length=max_length))
...
def main(
    ...
    if is_runtime:
        evaluate(model_dir, (FOX_DIR_PATH / FOX_INFO_FILENAME), max_length=max_length, is_csv=True,
                 title_col_num=FOX_TITLE_COLUMN, id_col_num=1)
    else:
