...
def loadTextFromFile(filepath, isText=True):
	try:
...
if __name__ == '__main__':
	...
	dump = False
	if len(sys.argv) > 1:
		if sys.argv[1] == 'p':# p flag to use pickeled data
			with open('pickled_matrices','rb') as f:
				[whole_doc,X_train,X_test, y_train, y_test] = pickle.load(f)
		elif sys.argv[1] == 'd': #d flag to dump pickled data
			dump = True
	else:
		...
		for n in range(14,26,1):
			train_text.append(loadTextFromFile(f'../texts/text-2019-09-{n}.csv', True))
	...
	# 	temp = list(zip(cv.get_feature_names(), np.sum(whole_doc.toarray(), axis=0)))
